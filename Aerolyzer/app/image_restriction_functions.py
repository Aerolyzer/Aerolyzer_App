...
class imgRestFuncs(object):
    ...
    Purpose:        The purpose of this function is to determine whether or not the size of
                    the image is less than or equal to 400kb.
    ...
    def is_res(self, imageWidth, imageLength):
        ...
        if (imageWidth >= self.criteria['imgWidthMin']) and (imageLength >= self.criteria['imgWidthMin']):
            if (imageWidth <= self.criteria['imgWidthMax']) and (imageLength <= self.criteria['imgWidthMax']):
                return True
        else:
            return False
